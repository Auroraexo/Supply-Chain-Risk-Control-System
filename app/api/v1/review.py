...
from app.api.deps import DBSession, DeciderUser
...
async def approve_review(request_id: str, review: ReviewDecision, db: DBSession, user: DeciderUser):
    """审核通过。"""
    try:
        service = DecisionService(db)
        result = await service.submit_review(
            request_id, "approve", user.get("sub", "unknown"), review.comment
        )
...
async def reject_review(request_id: str, review: ReviewDecision, db: DBSession, user: DeciderUser):
    """审核驳回。"""
    try:
        service = DecisionService(db)
        result = await service.submit_review(
            request_id, "reject", user.get("sub", "unknown"), review.comment
        )
        return DataResponse(data=result)
    except AppException as e:
        raise HTTPException(
...
async def override_decision(
    request_id: str, review: ReviewDecision, db: DBSession, user: DeciderUser
):
    """人工覆盖决策。"""
    try:
        service = DecisionService(db)
